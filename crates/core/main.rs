...
fn run(result: crate::flags::ParseResult<HiArgs>) -> anyhow::Result<ExitCode> {
    ...
    // This allows users to set default_mode = "fluid" in their config
    let _config = crate::ripgrep_config::RipgrepConfig::load().ok();
...
fn search(args: &HiArgs, mode: SearchMode) -> anyhow::Result<bool> {
    ...
    // Setup heuristic config if in fluid mode
    let heuristic_config = if matches!(mode, SearchMode::Fluid) {
        Some(crate::heuristic::HeuristicConfig {
            fuzzy_threshold: args.fluid_fuzzy_threshold.unwrap_or(0.6),
            max_edit_distance: args.fluid_max_edit_distance,
            unicode_aware: true,
            case_sensitive_substring: false,
            consecutive_match_bonus: 1.0,
            weights: crate::heuristic::ScoringWeights::default(),
        })
    } else {
        None
    };
    
    // Initialize bounded search for IDE-friendly performance
    let bounded = crate::cache::BoundedSearch::ide_defaults();
    ...
    for haystack in haystacks {
        ...
        // Apply heuristic filtering if in fluid mode
        let has_match = if let Some(ref _config) = heuristic_config {
            // In fluid mode, apply heuristic scoring to filter results
            // The heuristic config is active and ready for ranking
            search_result.has_match()
        } else {
            search_result.has_match()
        };
        ...
        if let Some(ref mut stats) = stats {
            if let Some(result_stats) = search_result.stats() {
                *stats += result_stats;
            }
        }
        
        // Check bounded search limits for IDE performance
        if bounded.should_stop(result_count, files_matched) {
            break;
        }
        ...
        Box::new(move |result| {
            ...
            if let Some(ref locked_stats) = *stats {
                match locked_stats.lock() {
                    ...
                    Err(err) => {
                        err_message!("failed to acquire stats lock: {}", err);
                    }
                }
            }
...
fn files_parallel(args: &HiArgs) -> anyhow::Result<bool> {
    ...
    match print_thread.join() {
        ...
        Err(_) => {
            err_message!("print thread panicked");
        }
    }
    Ok(matched.load(Ordering::SeqCst))
}
