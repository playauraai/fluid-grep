...
impl Flag for Fluid {
    ...
    fn doc_short(&self) -> &'static str {
        r"Enable fluid search mode."
    }
    fn doc_long(&self) -> &'static str {
        r"
Enable fluid search mode. This is a custom search mode that provides flexible
and adaptive matching capabilities using heuristic search algorithms.
.sp
In fluid mode, ripgrep will perform searches with:
.sp
.IP \(bu 3n
Fuzzy matching - finds patterns even with typos or partial matches
.sp
.IP \(bu 3n
Relevance scoring - ranks results by match quality
.sp
.IP \(bu 3n
Word boundary detection - prioritizes matches at word boundaries
.sp
.IP \(bu 3n
Consecutive character matching - scores patterns with consecutive matches higher
.sp
This mode is useful for exploratory searches where you want flexible pattern
matching and intelligent result ranking. Results are automatically sorted by
relevance score.
"
    }
    ...
    fn update(&self, v: FlagValue, args: &mut LowArgs) -> anyhow::Result<()> {
        if v.unwrap_switch() {
            args.mode.update(Mode::Search(SearchMode::Fluid));
        } else if matches!(args.mode, Mode::Search(SearchMode::Fluid)) {
            // --no-fluid only reverts to the default mode if the mode is
            // Fluid, otherwise it's a no-op.
...
fn test_fluid() {
    ...
    let args = parse_low_raw(["--fluid"]).unwrap();
    assert_eq!(Mode::Search(SearchMode::Fluid), args.mode);

    let args = parse_low_raw(["--fluid", "--no-fluid"]).unwrap();
    assert_eq!(Mode::Search(SearchMode::Standard), args.mode);

    let args = parse_low_raw(["--no-fluid"]).unwrap();
    assert_eq!(Mode::Search(SearchMode::Standard), args.mode);
}

/// --fluid-fuzzy-threshold
#[derive(Debug)]
struct FluidFuzzyThreshold;

impl Flag for FluidFuzzyThreshold {
    fn is_switch(&self) -> bool {
        false
    }
    fn name_long(&self) -> &'static str {
    ...
    fn doc_short(&self) -> &'static str {
        "Set fuzzy match threshold for fluid mode (0.0-1.0)."
    }
    fn doc_long(&self) -> &'static str {
        r"
Set the fuzzy match threshold for fluid search mode. This controls how
permissive fuzzy matching is. Default is 0.6 (60%).
.sp
.IP \(bu 3n
...
.sp
.IP \(bu 3n
1.0 = strict (require exact fuzzy match)
"
    }
    fn doc_variable(&self) -> Option<&'static str> {
        Some("THRESHOLD")
    ...
    fn update(&self, v: FlagValue, args: &mut LowArgs) -> anyhow::Result<()> {
        let threshold: f32 = convert::str(&v.unwrap_value())?.parse()?;
        if threshold < 0.0 || threshold > 1.0 {
            anyhow::bail!("fuzzy threshold must be between 0.0 and 1.0");
        }
        args.fluid_fuzzy_threshold = Some(threshold);
        Ok(())
    }
}
...
#[cfg(test)]
#[test]
fn test_fluid_fuzzy_threshold() {
    let args = parse_low_raw(None::<&str>).unwrap();
    assert_eq!(None, args.fluid_fuzzy_threshold);

    let args = parse_low_raw(["--fluid-fuzzy-threshold", "0.8"]).unwrap();
    assert_eq!(Some(0.8), args.fluid_fuzzy_threshold);

    let args = parse_low_raw(["--fluid-fuzzy-threshold=0.5"]).unwrap();
    assert_eq!(Some(0.5), args.fluid_fuzzy_threshold);
}

/// --fluid-max-edit-distance
#[derive(Debug)]
struct FluidMaxEditDistance;

impl Flag for FluidMaxEditDistance {
    fn is_switch(&self) -> bool {
        false
    }
    fn name_long(&self) -> &'static str {
    ...
    fn doc_short(&self) -> &'static str {
        "Set max edit distance for typo tolerance in fluid mode."
    }
    fn doc_long(&self) -> &'static str {
        r"
Set the maximum edit distance (Levenshtein distance) for typo tolerance
in fluid search mode. This controls how many character edits are allowed
when matching. Default is pattern.len() / 4.
.sp
.IP \(bu 3n
0 = no typos allowed (exact match)
.sp
.IP \(bu 3n
...
"
    ...
    fn update(&self, v: FlagValue, args: &mut LowArgs) -> anyhow::Result<()> {
        let distance: usize = convert::str(&v.unwrap_value())?.parse()?;
        args.fluid_max_edit_distance = Some(distance);
        Ok(())
    }
}
...
#[cfg(test)]
#[test]
fn test_fluid_max_edit_distance() {
    let args = parse_low_raw(None::<&str>).unwrap();
    assert_eq!(None, args.fluid_max_edit_distance);

    let args = parse_low_raw(["--fluid-max-edit-distance", "2"]).unwrap();
    assert_eq!(Some(2), args.fluid_max_edit_distance);

    let args = parse_low_raw(["--fluid-max-edit-distance=0"]).unwrap();
    assert_eq!(Some(0), args.fluid_max_edit_distance);
}
