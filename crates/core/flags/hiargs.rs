...
pub(crate) struct HiArgs {
    ...
    fixed_strings: bool,
    pub(crate) fluid_fuzzy_threshold: Option<f32>,
    pub(crate) fluid_max_edit_distance: Option<usize>,
    pub(crate) wsl2_optimized: bool,
    follow: bool,
...
impl HiArgs {
    ...
    pub(crate) fn from_low_args(mut low: LowArgs) -> anyhow::Result<HiArgs> {
        ...
        Ok(HiArgs {
            ...
            fluid_fuzzy_threshold: low.fluid_fuzzy_threshold,
            fluid_max_edit_distance: low.fluid_max_edit_distance,
            wsl2_optimized: cfg!(target_os = "linux"),
            follow: low.follow,
            heading,
            hidden: low.hidden,
            hyperlink_config,
    ...
    pub(crate) fn printer<W: termcolor::WriteColor>(
        ...
        let summary_kind = if self.quiet {
            ...
            match search_mode {
                ...
                SearchMode::Fluid | SearchMode::Standard => {
...
fn globs(
    ...
    if low.glob_case_insensitive {
        builder.case_insensitive(true)
            .map_err(|err| anyhow::anyhow!("failed to set case insensitive: {}", err))?;
    }
...
fn stats(low: &LowArgs) -> Option<grep::printer::Stats> {
    ...
    if low.stats || matches!(low.mode, Mode::Search(SearchMode::JSON | SearchMode::Fluid)) {
