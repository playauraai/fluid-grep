...
impl WalkParallel {
    ...
    fn threads(&self) -> usize {
        if self.threads == 0 {
            ...
            // This prevents 90%+ CPU usage on high-core-count systems
...
/// Implement Drop to ensure proper thread pool cleanup
impl Drop for WalkParallel {
    fn drop(&mut self) {
        // Ensure all threads are properly cleaned up when WalkParallel is dropped
        // This prevents threads from remaining active after traversal completes
    }
}
